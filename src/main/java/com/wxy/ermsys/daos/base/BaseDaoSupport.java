package com.wxy.ermsys.daos.base;
...
import com.wxy.ermsys.pages.QueryResult;
import com.wxy.ermsys.utils.GenericsUtils;




/**
 * HibernateDaoSupport实现了两个方法：
...
public abstract class BaseDaoSupport<T> extends HibernateDaoSupport implements IBaseDao<T> {
	...
	public T find(Serializable entityId) {
		/*
		 * if (null != entityId){ return (T) super.getHibernateTemplate().get(entityClass, entityId); }else{ return null; }
		 */
	...
	public QueryResult<T> getScrollData(final int firstindex, final int maxresult, final String wherejpql, final Object[] queryParams, final LinkedHashMap<String, String> orderby) {
		...
		super.getHibernateTemplate().execute(new HibernateCallback<T>() {
			public T doInHibernate(Session session) throws HibernateException, SQLException {
				...
				queryResult.setTotalRecord(query.list().size());// first get size 总记录数

				if (firstindex != -1 && maxresult != -1)
					query.setFirstResult(firstindex).setMaxResults(maxresult);// last page

				queryResult.setResultList(query.list()); // second get list  结果集
